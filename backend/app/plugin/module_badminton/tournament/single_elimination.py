...
class SingleEliminationEngine(TournamentEngine):
    ...
    def generate_bracket(self, participants: List[Participant], num_participants: int) -> List[Match]:
        """生成对应人数的淘汰赛对阵表"""
        matches = []
        ...
        if num_participants == 3:
            ...
            matches.append(self.create_match(seeded_participants[1], seeded_participants[2], 1, 1))
            # 决赛（假设第一场胜者）
            # 需要动态生成，这里只生成第一轮
            
        elif num_participants == 4:
            # 4人小组：1vs4，2vs3
            matches.append(self.create_match(seeded_participants[0], seeded_participants[3], 1, 1))
            matches.append(self.create_match(seeded_participants[1], seeded_participants[2], 1, 2))
            # 决赛和季军赛需要根据结果动态生成
            
        elif num_participants == 5:
            # 5人小组：4vs5（资格赛），胜者vs1，2vs3
            matches.append(self.create_match(seeded_participants[3], seeded_participants[4], 1, 1))  # 资格赛
            # 后续比赛需要动态生成
            
        elif num_participants == 6:
            # 6人小组：3vs6，4vs5（资格赛）
            matches.append(self.create_match(seeded_participants[2], seeded_participants[5], 1, 1))
            matches.append(self.create_match(seeded_participants[3], seeded_participants[4], 1, 2))
            # 后续比赛需要动态生成
            ...
            for i in range(0, next_power_of_two // 2):
                ...
                if player1_idx < num_participants and player2_idx < num_participants:
                    matches.append(self.create_match(
    ...
    def create_match(self, player1: Participant, player2: Participant, round_num: int, match_num: int) -> Match:
        """创建比赛对象"""
        return Match(
            id=len(self._match_counter) + 1 if hasattr(self, '_match_counter') else match_num,
