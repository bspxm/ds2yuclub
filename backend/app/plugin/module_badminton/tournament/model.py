...
class TournamentModel(ModelMixin, UserMixin):
    ...
    registration_deadline: Mapped[datetime | None] = mapped_column(
        DateTime, nullable=True, comment="报名截止时间"
    )

    # 赛制参数
    max_participants: Mapped[int | None] = mapped_column(
        Integer, nullable=True, comment="最大参赛人数"
    )
    group_size: Mapped[int | None] = mapped_column(
        SmallInteger, nullable=True, comment="每组人数"
    )
    num_groups: Mapped[int | None] = mapped_column(
        SmallInteger, nullable=True, comment="分组数量"
    )
    match_format: Mapped[str | None] = mapped_column(
        String(32), nullable=True, comment="比赛形式（如三局两胜）"
    )
    points_per_game: Mapped[int | None] = mapped_column(
    ...
    location: Mapped[str | None] = mapped_column(
        String(255), nullable=True, comment="比赛地点"
    )
...
class TournamentGroupModel(ModelMixin):
    ...
    group_order: Mapped[int] = mapped_column(
        SmallInteger, nullable=False, comment="组序"
    )
...
class TournamentParticipantModel(ModelMixin):
    ...
    group_id: Mapped[int | None] = mapped_column(
        Integer,
        ForeignKey(
            "badminton_tournament_group.id", ondelete="SET NULL", onupdate="CASCADE"
        ),
        nullable=True,
        index=True,
        comment="分组ID",
    ...
    # 参赛信息
    seed_rank: Mapped[int | None] = mapped_column(
        Integer, nullable=True, comment="种子排名"
    )
    final_rank: Mapped[int | None] = mapped_column(
        Integer, nullable=True, comment="最终排名"
    )
    ...
    matches_won: Mapped[int] = mapped_column(
        Integer, default=0, nullable=False, comment="胜场"
    )
    matches_lost: Mapped[int] = mapped_column(
        Integer, default=0, nullable=False, comment="负场"
    )
    total_points_scored: Mapped[int] = mapped_column(
        Integer, default=0, nullable=False, comment="总得分"
    )
    total_points_conceded: Mapped[int] = mapped_column(
        Integer, default=0, nullable=False, comment="总失分"
    )
...
class TournamentMatchModel(ModelMixin):
    ...
    __table_args__: tuple = (
        Index("ix_h2h_lookup", "player1_id", "player2_id"),
        {"comment": "比赛对阵表"},
    )
    ...
    # 轮次类型
    round_type: Mapped[RoundTypeEnum] = mapped_column(
        Enum(RoundTypeEnum, values_callable=lambda x: [e.value for e in x]),
        default=RoundTypeEnum.GROUP_STAGE,
        nullable=False,
        comment="轮次类型",
    )
    ...
    round_number: Mapped[int] = mapped_column(
        SmallInteger, nullable=False, comment="轮次"
    )
    ...
    scheduled_time: Mapped[datetime | None] = mapped_column(
        DateTime, nullable=True, comment="计划时间"
    )
