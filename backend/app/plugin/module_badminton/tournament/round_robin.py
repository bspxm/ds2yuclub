...
from .engine import (
    Group, Match, MatchFormat, Participant, TournamentConfig,
    TournamentEngine, TournamentResult, TournamentType,
...
class RoundRobinEngine(TournamentEngine):
    ...
    def create_groups(self, participants: List[Participant]) -> List[Group]:
        ...
        for i in range(num_groups):
            ...
            groups.append(Group(id=i+1, name=group_name, participants=[], order=i+1))
    ...
    def generate_knockout_matches(self, groups: List[Group], group_results: Dict[int, List[Participant]]) -> List[Match]:
        """
        生成交叉淘汰赛对阵表
        ...
        # 种子排序（按小组排名）
        seeded_participants = sorted(advancing_participants, 
                                   key=lambda p: getattr(p, 'group_rank', 999))
        ...
        for i in range(0, next_power_of_two // 2):
            ...
            if player1_idx < len(seeded_participants) and player2_idx < len(seeded_participants):
    ...
    def calculate_rankings(self, groups: List[Group], matches: List[Match]) -> Dict[int, List[Participant]]:
        ...
        for group in groups:
            group_matches = [m for m in matches 
                           if (m.player1_id in [p.id for p in group.participants] or 
                               m.player2_id in [p.id for p in group.participants])]
            ...
            for match in group_matches:
                if match.winner_id:
                    ...
                    if match.winner_id in participant_stats and loser_id in participant_stats:
                        # 记录胜负关系
                        if loser_id not in participant_stats[match.winner_id]['head_to_head']:
                            participant_stats[match.winner_id]['head_to_head'][loser_id] = 0
                        if match.winner_id not in participant_stats[loser_id]['head_to_head']:
                            participant_stats[loser_id]['head_to_head'][match.winner_id] = 0
                        
                        participant_stats[match.winner_id]['head_to_head'][loser_id] += 1
            ...
            while i < len(sorted_pids):
                ...
                while j < len(sorted_pids):
                    ...
                    if (current_stats['wins'] == next_stats['wins'] and 
                        current_stats['points'] == next_stats['points']):
                ...
                if len(same_score_group) == 1:
                    ...
                    # 处理积分相同的选手
                    # 按相互胜负关系排序
                    tied_participants = [(pid, participant_stats[pid]) for pid in same_score_group]
    ...
    def simulate_group_stage(self, groups: List[Group], matches: List[Match]) -> List[Match]:
        """
        模拟小组赛（用于测试）
