...
class TournamentCreateSchema(BaseModel):
    ...
    registration_deadline: Optional[DateTimeStr] = Field(None, description="报名截止时间")
    ...
    @classmethod
    def validate_registration_deadline(cls, v: Optional[DateTimeStr], info) -> Optional[DateTimeStr]:
        """验证报名截止时间"""
        if v is None:
            return v
        start_date = info.data.get('start_date')
...
class TournamentOutSchema(TournamentCreateSchema, BaseSchema, UserBySchema):
    """赛事响应模型"""
    status: TournamentStatusEnum = Field(default=TournamentStatusEnum.DRAFT, description="赛事状态")
