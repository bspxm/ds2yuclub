...
class PurchaseService:
    ...
    @classmethod
    async def get_remaining_sessions_cached(
        cls, auth: AuthSchema, redis: Redis, student_id: int, class_id: int
    ) -> int:
        """
        获取学员剩余课时（带Redis缓存）
        ...
        purchases = await PurchaseCRUD(auth).list(
            search={"student_id": ("eq", student_id), "class_id": ("eq", class_id)}
        )
        ...
        # 存入Redis缓存
        await BadmintonCache.set_json(
            redis, cache_key, remaining_sessions, CacheExpireTime.STUDENT_SESSIONS
        )
    ...
    async def detail_service(cls, auth: AuthSchema, purchase_id: int) -> dict:
        ...
        if purchase.created_id:
            related_queries.append(
                ("creator", UserCRUD(auth).get_by_id_crud(id=purchase.created_id))
            )

        if purchase.updated_id:
            related_queries.append(
                ("updater", UserCRUD(auth).get_by_id_crud(id=purchase.updated_id))
            )

        # 等待查询完成
        related_results = await asyncio.gather(
            *[r[1] for r in related_queries], return_exceptions=True
        )

        # 提取结果
        created_by = None
        updated_by = None
        for i, (query_name, result) in enumerate(
            zip([r[0] for r in related_queries], related_results)
        ):
        ...
        item = {
            ...
            "settlement_date": purchase.settlement_date.isoformat()
            if purchase.settlement_date
            else None,
            # 财务信息
            ...
            "created_time": purchase.created_time.isoformat()
            if purchase.created_time
            else None,
            "updated_time": purchase.updated_time.isoformat()
            if purchase.updated_time
            else None,
            # 视图字段 - 学员信息
            "student": {"id": purchase.student_id, "name": purchase.student_name}
            if purchase.student_name
            else None,
            # 视图字段 - 班级信息
            "class": {"id": purchase.class_id, "name": purchase.class_name}
            if purchase.class_name
            else None,
            # 视图字段 - 学期信息
            "semester": {"id": purchase.semester_id, "name": purchase.semester_name}
            if purchase.semester_name
            else None,
            # 视图字段 - 教练信息
            "coach_user": {
            ...
            if purchase.coach_user_name
            else None,
            # 创建人和更新人
            "created_by": {"id": created_by.id, "name": created_by.name}
            if created_by
            else None,
            "updated_by": {"id": updated_by.id, "name": updated_by.name}
            if updated_by
            else None,
        }
    ...
    async def page_service(
        ...
        page_no: int,
        page_size: int,
        search: Optional[dict | PurchaseQueryParam] = None,
        order_by: Optional[list[dict]] = None,
    ) -> dict:
        """购买记录分页查询（使用视图优化性能）"""
        ...
        logger.info(
            f"购买记录分页查询耗时（使用视图）: {page_end - page_start:.3f}秒, 结果数: {len(result['items'])}"
        )
    ...
    async def create_service(
        cls, auth: AuthSchema, redis: Redis, data: PurchaseCreateSchema
    ) -> dict:
        """创建购买记录（带缓存失效）"""
        # 检查学员是否已购买该班级（同一学期内）
        # TODO: 实现重复购买检查
        ...
        if purchase_data.get("selected_time_slots") is not None:
            purchase_data["selected_time_slots"] = json.dumps(
                purchase_data["selected_time_slots"]
            )
                ...
                carry_over_details.append(
                    {
                        ...
                        "previous_semester": prev_purchase.semester.name
                        if prev_purchase.semester
                        else "未知学期",
                        "carry_over_sessions": prev_purchase.remaining_sessions,
        ...
        # 计算并设置剩余课时
        purchase_data["remaining_sessions"] = purchase_data.get(
            "total_sessions", 0
        ) - purchase_data.get("used_sessions", 0)
        ...
        # 失效学员课时缓存
        cache_key = (
            f"{BadmintonCacheKeys.STUDENT_SESSIONS}:{data.student_id}:{data.class_id}"
        )
    ...
    async def batch_create_service(
        cls, auth: AuthSchema, redis: Redis, data: BatchPurchaseCreateSchema
    ) -> dict:
        """批量创建购买记录"""
        results = []
        success_count = 0
        ...
        for student_id in data.student_ids:
            try:
                ...
                logger.error(
                    f"批量创建购买记录失败 - 学员ID: {student_id}, 错误: {str(e)}"
                )
                results.append(
                    {"student_id": student_id, "success": False, "error": str(e)}
                )
                failed_count += 1
    ...
    async def update_service(
        ...
        if update_data.get("selected_time_slots") is not None:
            update_data["selected_time_slots"] = json.dumps(
                update_data["selected_time_slots"]
            )
        ...
        # 计算剩余课时
        total_sessions = update_data.get(
            "total_sessions", original_purchase.total_sessions
        )
        used_sessions = update_data.get(
            "used_sessions", original_purchase.used_sessions
        )
        update_data["remaining_sessions"] = total_sessions - used_sessions

        purchase = await PurchaseCRUD(auth).update_crud(
            id=purchase_id, data=update_data
        )
        if not purchase:
    ...
    async def settle_purchase_service(cls, auth: AuthSchema, purchase_id: int) -> dict:
        ...
        settled_purchase = await PurchaseCRUD(auth).update_crud(
            id=purchase_id, data=PurchaseUpdateSchema(**update_data)
        )
        ...
        return SimpleResponse(
            ...
            message="购买记录结算成功",
            data={
                "purchase": PurchaseOutSchema.model_validate(
                    settled_purchase
                ).model_dump(),
                "settlement_summary": {
                    "total_sessions": purchase.total_sessions,
    ...
    @classmethod
    async def settle_semester_purchases_service(
        cls, auth: AuthSchema, semester_id: int
    ) -> dict:
        """结算指定学期的所有购买记录"""
        from datetime import date
        from ..enums import PurchaseStatusEnum
        ...
        for purchase in purchases:
            ...
            settled_purchase = await PurchaseCRUD(auth).update_crud(
                id=purchase.id, data=PurchaseUpdateSchema(**update_data)
            )
                ...
                settlement_results.append(
                    {
                        ...
                        "student_name": purchase.student.name
                        if purchase.student
                        else "未知学员",
                        "class_name": purchase.class_ref.name
                        if purchase.class_ref
                        else "未知班级",
                        "total_sessions": purchase.total_sessions,
                        "used_sessions": purchase.used_sessions,
                        "remaining_sessions": remaining_sessions,
        ...
        return SimpleResponse(
            ...
            data={
                ...
                "summary": {
                    ...
                    "平均每学员剩余课时": total_remaining / len(successful_settlements)
