        ...
        >
              ...
              <div v-if="scope.row.time_slots_json" class="time-slots-container">
                <div
                  v-for="(dayGroup, index) in formatSelectedTimeSlots(
                    JSON.parse(scope.row.time_slots_json)
                  )"
                  :key="index"
                  class="day-group"
                >
          ...
          <el-table-column label="排课状态" prop="schedule_status" min-width="90">
            <template #default="scope">
              <el-tag
                :type="
                  scope.row.schedule_status === 'scheduled'
                    ? 'info'
                    : scope.row.schedule_status === 'active'
                      ? 'success'
                      : scope.row.schedule_status === 'completed'
                        ? 'warning'
                        : 'danger'
                "
              >
        ...
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="时间段配置" :span="2">
            ...
            <span v-else-if="detailFormData.time_slot_code">
              {{ detailFormData.time_slot_code }}时段 ({{ detailFormData.start_time }} -
          ...
          <el-descriptions-item label="时长">
            {{
              detailFormData.duration_minutes
          ...
          <el-descriptions-item label="排课状态">
            <el-tag
              :type="
                detailFormData.schedule_status === 'scheduled'
                  ? 'info'
                  : detailFormData.schedule_status === 'active'
                    ? 'success'
                    : detailFormData.schedule_status === 'completed'
                      ? 'warning'
                      : 'danger'
              "
            >
          ...
          <el-row :gutter="20">
            ...
            <el-col :span="10">
              <el-card class="student-list-card">
                ...
                <div
                  v-else-if="!loadingStudents && filteredStudents.length === 0"
                  class="empty-tip"
                >
                    ...
                    <div class="student-item">
                      ...
                      <div class="student-info">
                        <el-tag size="small" type="info">
                          {{
                            student.birth_date
                              ? calculateAge(student.birth_date) + "岁"
                              : "未知年龄"
                          }}
                        </el-tag>
                        ...
                        <el-tag
                          size="small"
                          :type="student.remaining_sessions > 0 ? 'success' : 'danger'"
                        >
                          剩余: {{ student.remaining_sessions }} / 总计:
      ...
      <template #footer>
        <div class="dialog-footer">
          ...
          <el-button
            ...
            @click="handleSubmit"
          >
            <template v-if="!submitLoading">确定</template>
          </el-button>
...
import ClassScheduleAPI, {
  ClassScheduleTable,
  ClassSchedulePageQuery,
  AvailableStudentInfo,
  ClassScheduleCreateV2Form,
  TimeSlotInfo,
  DictDataItem,
} from "@/api/module_badminton/class-schedule";
...
const uniqueGroups = computed(() => {
  const groups = availableStudents.value.map((s) => s.group_name).filter((g): g is string => !!g);
  return [...new Set(groups)].sort();
});

// 获取唯一的水平列表
const uniqueLevels = computed(() => {
  const levels = availableStudents.value.map((s) => s.level).filter((l): l is string => !!l);
...
async function loadAvailableStudents() {
  ...
  if (
    !formDataV2.semester_id ||
    !formDataV2.schedule_date ||
    formDataV2.class_ids.length === 0 ||
    tempTimeSlotIds.value.length === 0
  ) {
    availableStudents.value = [];
    // 新增模式下清空，编辑模式下保留
    if (!editingScheduleId.value) {
...
async function loadAvailableTimeSlots() {
  ...
  try {
    ...
    results.forEach((response, index) => {
      ...
      if (response.data && response.data.data) {
        ...
        if (response.data.data.time_slots) {
          console.log(
            `班级 ${formDataV2.class_ids[index]} 的 time_slots:`,
            response.data.data.time_slots
          );
    ...
    console.log(
      `过滤 ${dayOfWeekName} 的时间段，过滤前: ${allTimeSlots.length} 个，过滤后: ${filteredTimeSlots.length} 个`
    );
...
// 监听班级和日期变化，重新加载可用时间段
watch(
  () => [formDataV2.class_ids, formDataV2.schedule_date],
  () => {
...
async function handleOpenDialog(type: "create" | "update" | "detail", id?: number) {
  ...
  if (id) {
    ...
    try {
      // 并行加载基础数据
      const loadPromises = [loadSemesters(), loadCoaches(), loadTimeSlotDict()];
    ...
    // 加载V2版本所需的数据
    await Promise.all([loadSemesters(), loadCoaches(), loadTimeSlotDict()]);
  }
}
...
onMounted(() => {
  ...
  Promise.all([loadSemesters(), loadCoaches(), loadTimeSlotDict()]).catch((error) => {
