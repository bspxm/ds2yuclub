  ...
  <div class="app-container">
      ...
      >
        ...
        <el-form-item prop="class_type" label="班级类型">
          ...
          >
            ...
            <el-option value="optional" label="自选天" />
          </el-select>
        </el-form-item>
    ...
    <el-card class="data-table">
      ...
      >
        ...
        >
          <template #default="scope">
            <el-tag :type="scope.row.current_students >= scope.row.max_students ? 'danger' : 'success'">
        ...
        <el-table-column
          ...
          label="每节课费用"
          prop="fee_per_session"
          min-width="100"
        >
          <template #default="scope">
            ¥{{ scope.row.fee_per_session }}
          </template>
        </el-table-column>
        <el-table-column
        ...
        >
          <template #default="scope">
            <el-tag :type="scope.row.status === 'active' ? 'success' : scope.row.status === 'pending' ? 'info' : 'warning'">
              {{ scope.row.status === 'active' ? '进行中' : scope.row.status === 'pending' ? '未开始' : '已结束' }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column
      ...
      <template v-if="dialogVisible.type === 'detail'">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="班级状态">
            <el-tag :type="detailFormData.status === 'active' ? 'success' : detailFormData.status === 'pending' ? 'info' : 'warning'">
              {{ detailFormData.status === 'active' ? '进行中' : detailFormData.status === 'pending' ? '未开始' : '已结束' }}
            </el-tag>
          </el-descriptions-item>
          ...
          <el-descriptions-item label="上课时间">
            {{ detailFormData.class_time || '未设置' }}
          </el-descriptions-item>
          <el-descriptions-item label="课程时长">
            {{ detailFormData.duration_minutes ? `${detailFormData.duration_minutes}分钟` : '未设置' }}
          </el-descriptions-item>
          <el-descriptions-item label="每周排班">
            {{ detailFormData.weekly_schedule || '未设置' }}
          </el-descriptions-item>
          ...
          <el-row :gutter="20">
            ...
            <el-col :span="12">
              <el-form-item label="课程时长(分钟)">
                <el-input-number v-model="formData.duration_minutes" :min="30" :max="240" :step="15" placeholder="课程时长" style="width: 100%" />
              </el-form-item>
            </el-col>
            ...
            <el-col :span="12">
              <el-form-item label="每周排班">
                <el-input v-model="formData.weekly_schedule" placeholder="如：周一、周三、周五" :maxlength="128" />
              </el-form-item>
            </el-col>
            <el-col :span="12">
            ...
            <el-col :span="12">
              <el-form-item label="班级状态" prop="status">
                <el-select v-model="formData.status" placeholder="请选择班级状态" style="width: 100%">
                  <el-option value="pending" label="未开始" />
                  <el-option value="active" label="进行中" />
                  <el-option value="ended" label="已结束" />
                </el-select>
...
const tableColumns = ref([
  ...
  { prop: "fee_per_session", label: "每节课费用", show: true },
